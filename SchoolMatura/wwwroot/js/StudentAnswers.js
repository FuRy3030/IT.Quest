class DataManagement {
    ...
    static SubmitCheckedAnswers(MarkedExercises, GradingStatus) {
        const QueryString = window.location.search;
        const URLParams = new URLSearchParams(QueryString);
        const Identifier = URLParams.get('Identifier');
...
async function InitialLoad() {
    ...
    for (let i = 0; i < AnswersData.Answers.length; i++) {
        ...
        if (AnswersData.Answers[i].GainedPoints != -1) {
    ...
    for (let i = 0; i < AnswersData.Answers.length; i++) {
        if (AnswersData.Answers[i].ExerciseType == 'TrueFalseExercise') {
            if (AnswersData.Answers[i + 1] == undefined || 
                AnswersData.Answers[i + 1].ExerciseType != 'TrueFalseExercise') 
            {
    ...
    $SaveButton.on('click', function(event) {
        ...
        if (MarkedExercises.every((Exercise) => Exercise.AnswerPoints == -1)) {
            DataManagement.SubmitCheckedAnswers(MarkedExercises, 0);
        }
        else if (MarkedExercises.every((Exercise) => Exercise.AnswerPoints != -1)) {
            DataManagement.SubmitCheckedAnswers(MarkedExercises, 2);
        }
        else {
            DataManagement.SubmitCheckedAnswers(MarkedExercises, 1);
        }
