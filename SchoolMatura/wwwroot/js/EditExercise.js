class DataManagement {
    ...
    static DeleteCurrentExercises(ExercisesIdentifiers) {
        return new Promise(function(resolve, reject) {
            $.ajax({
...
class RenderContent {
    ...
    static CreateQuestion(QuestionContent = '', QuestionType = 'default', QuestionTypeForDisplay, Points = '', 
        Hashtags = '', AdditionalData = '') 
    {
    ...
    static CreateQuestionTrueFalse(QuestionContent = '', QuestionSubNumber, QuestionMainNumber, UniqueClass, $Node = null, 
        Points = '', Hashtags = '', CorrectAnswer = '1') 
    {
...
async function InitialLoad() {
    try {
        const CurrentQuestionData = await DataManagement.LoadQuestionData();
        let UniqueNumber = Date.now();
        let Counter = 1;
        CurrentQuestionData.forEach(exercise => {
            switch (exercise.ExerciseType) {
                case 'StandardExercise':
                    RenderContent.CreateQuestion(exercise.Content, 'default', 'Zwykłe Otwarte', exercise.Points, 
                ...
                case 'ProgrammingExercise':
                    RenderContent.CreateQuestion(exercise.Content, 'programming', 'Programistyczne', exercise.Points, 
                        exercise.Hashtags == null ? '' : exercise.Hashtags, exercise.AdditionalData);

                    UpdateBottomNavbar('programistycznego');
                    break;
                case 'TrueFalseExercise':
                    RenderContent.CreateQuestionTrueFalse(exercise.Content, Counter, 1, UniqueNumber, null, 
                        exercise.Points, exercise.Hashtags == null ? '' : exercise.Hashtags, exercise.CorrectAnswer);
                    
                    UpdateBottomNavbar('typu prawda - fałsz');
                    RenderContent.QuestionOrder = 2;
                    Counter = Counter + 1;
                    break;
            }
        });

        $('.col-12 .standard-input-group input').first().val(CurrentQuestionData[0].Title);  
        ControlEmptyNotificationDisplay();    
    }
    catch (ex) {
        console.log(ex);
...
jQuery(function () {
    ...
    $SaveSetButton.on('click', function(event) {
        ...
        DataManagement.DeleteCurrentExercises([Identifier]).then(response => {
            if (response == 'Success') {
