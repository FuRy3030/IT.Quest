class DataManagement {
    ...
    static UpdateCurrentSession(title, date, name) {
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.toString() == 'WrongDate') {
                    RenderContent.ShowErrorModal(`Wskazana data nie może być wcześniejsza niż obecna! Wybierz nową i spróbuj ponownie.`);
                }
...
class RenderContent {
    static CreateExerciseShortcut(type, content, points, hashtags, order, suborder = '') {
        ...
        Exercise.innerHTML = `<div class="exercise-details">
            <h4 class="question-order">${order}.${suborder}</h4>
...
jQuery(function() {
    ...
    $UpdateSessionButton.on('click', function(event) {
        event.preventDefault();
        const DateYearMonthDay = $('#DatePicker span').first().text();
        const Time = $('#DatePicker span').last().text();
        ...
        const SessionDate = new Date(Year, Month, Day, Hour, Minutes);
        const SessionName = $('#SessionName').val().trim();

        DataManagement.UpdateCurrentSession(localStorage.getItem('CurrentSetToOpen'), SessionDate, SessionName);
    });
